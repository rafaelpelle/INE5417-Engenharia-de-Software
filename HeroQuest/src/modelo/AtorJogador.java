...
public class AtorJogador extends JFrame {
	...
	public AtorJogador() {
		...
		ImageIcon iconeConectar = new ImageIcon(getClass().getResource("/imagens/BotaoConectar.png"));
		...
		ImageIcon iconeDesconectar = new ImageIcon(getClass().getResource("/imagens/BotaoDesconectar.png"));
		...
		ImageIcon iconeIniciarPartida = new ImageIcon(getClass().getResource("/imagens/BotaoIniciarPartida.png"));
		...
		ImageIcon iconeFinalizarJogada = new ImageIcon(getClass().getResource("/imagens/BotaoFinalizarJogada.png"));
		...
		ImageIcon iconeBotaoMostrarInventario = new ImageIcon(getClass().getResource("/imagens/BotaoMostrarInventario.png"));
		...
		ImageIcon iconeBotaoAtacar = new ImageIcon(getClass().getResource("/imagens/BotaoAtacar.png"));
		...
		ImageIcon iconeBotaoUsarMagia = new ImageIcon(getClass().getResource("/imagens/BotaoUsarMagia.png"));
		...
		ImageIcon iconeBotaoProcurarArmadilha = new ImageIcon(getClass().getResource("/imagens/BotaoProcurarArmadilha.png"));
		...
		ImageIcon iconeBotaoProcurarTesouro = new ImageIcon(getClass().getResource("/imagens/BotaoProcurarTesouro.png"));
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public Spell selecionarMagia(ArrayList<Spell> magiasDisponiveis) {
		...
		for (int i = 0;i < magiasDisponiveis.size();i++) {
			inputDialog += "\n"+i+" - "+magiasDisponiveis.get(i).getNome();
		}
	...
	public Creature selecionarAlvo(ArrayList<Creature> possiveisAlvos) {
		...
		for (int i = 0;i < possiveisAlvos.size();i++) {
			inputDialog += "\n"+i+" - "+possiveisAlvos.get(i).getClass().getSimpleName();
		}
		String opcao = JOptionPane.showInputDialog(inputDialog);
		int index = Integer.parseInt(opcao);
	...
	public void atualizarInterfaceGrafica() {
		for (int i=0; i<27; i++) {
			for (int j=0; j<50; j++){
				Position posicao = this.heroQuest.getPositionOfTheMap(i, j);
	...
	public void procurarArmadilhaOuPortaSecreta() {
		this.heroQuest.procurarArmadilha();
	}
	...
	public String obterIdJogador() {
		String idJogador = JOptionPane.showInputDialog("Digite o nome do seu personagem: ");
		return idJogador;
	}
	...
	/*resultado
	0 - conexao com exito
	1 - tentativa de conexao com conexao previamente estabelecida
	2 - tentativa de conexao falhou
	*/ 
	public void conectar() {
		boolean conectado = this.heroQuest.getConnected();
		if (!conectado){
			String servidor = this.obterDadosConexao();
			String idUsuario = obterIdJogador();
			boolean exito = this.heroQuest.atorClienteServidor.conectar(servidor, idUsuario);
			if (exito){
	...
	public void notificarResultado(int resultado) {
		String mensagem = "";
		switch(resultado) {
		case 0: mensagem = "Conexao com exito";
				break;
		case 1: mensagem = "Tentativa de conexao com conexao previamente estabelecida";
				break;
		case 2: mensagem = "Tentativa de conexao falhou";
				break;
		case 3: mensagem = "Desconexao com exito";
				break;
		case 4: mensagem = "Tentativa de desconexao sem conexao previamente estabelecida";
				break;
		case 5: mensagem = "Tentativa de desconexao falhou";
				break;
		case 6: mensagem = "Solicita��o de inicio procedida com exito";
				break;
		case 7: mensagem = "Tentativa de inicio sem conexao previamente estabelecida";
				break;
		case 13: mensagem = "Partida corrente nao interrompida";
				break;
		default: mensagem = "";
				break;
		}
	...
	/*resultado
	3 - desconexao com exito
	4 - tentativa de desconexao sem conexao previamente estabelecida
	5 - tentativa de desconexao falhou
	*/
	...
	/*resultado
	6 - solicita��o de inicio procedida com exito
	7 - tentativa de inicio sem conexao previamente estabelecida
	13 - partida corrente nao interrompida
	*/
	public void iniciarPartida() {
		boolean conectado = false;
		boolean interromper = false;
		boolean emAndamento = this.heroQuest.getInProgress();
		if (emAndamento){
			interromper = this.avaliarInterrupcao();
		}else{
			conectado = this.heroQuest.getConnected();
		}
		if (interromper || ((!emAndamento) && conectado)){
			int numJog = this.informarQuantidadeDePlayers();
			this.heroQuest.startMatch(numJog);
	...
	public void anunciarVitoriaDosJogadores() {
		JOptionPane.showMessageDialog(null, "Parab�ns aos aventureiros!\n Voc�s foram vitoriosos!!!");
	}

	public void anunciarVitoriaDoZargon() {
		JOptionPane.showMessageDialog(null, "Oh n�o, o terr�vel Zargon venceu desta vez!");
	}

	public String informarNomeJogador(){
		String nomeJogador = JOptionPane.showInputDialog("Digite o nome do seu personagem: ");
		return nomeJogador;
	}
	...
	public void mostrarInventario(int gold) {
		JOptionPane.showMessageDialog(null, "Voc� possui "+gold+" gold coins no seu invent�rio.");
	}
	...
	public void mostrarInformacoes(int body, int mind, int movement, Status status) {
		JOptionPane.showMessageDialog(null, "Body atual: "+body+"\nMind restante: "+mind+"\nMovimento restante: "+movement+"\nStatus atual: "+status);
	}

	public int informarQuantidadeDePlayers() {
		int numJogadores;
		do {
			String numjog = JOptionPane.showInputDialog("Favor digitar a quantidade de jogadores que ir�o participar desta partida: ");
	...
	private void atualizarBotao(JButton botao, Position posicao) {
		ImageIcon img;
		String path = "";
		int linha = posicao.getRow();
		int coluna = posicao.getColumn();
		if (posicao.getCreature() != null) {
			path = "/imagens/" + posicao.getCreature().getClass().getSimpleName() + ".png";
			
		} else if(posicao.getTrap() != null) {
			if (posicao.getTrap().getVisible()) {
				path = "/imagens/" + posicao.getTrap().getClass().getSimpleName() + ".png";
			} else {
				path = "/imagens/" + posicao.getClass().getSimpleName() + ".png";
			}
		} else {
	...
	private void atualizarBotao(JButton botao, Creature criatura, int posicaoBotao) {
	...
	public void mostrarAcaoTrap(int dano, Creature criatura) {
		JOptionPane.showMessageDialog(null, "Oh n�o! "+criatura.getClass().getSimpleName()+" ativou uma armadilha e perdeu "+dano+" de body points!");
	}

	public void mostrarDano(Creature alvo, int dano) {
		JOptionPane.showMessageDialog(null, "A criatura "+alvo.getClass().getSimpleName()+" recebeu "+dano+" de dano.");
	}

	public void anunciarMorteDeCriatura(Creature alvo) {
		JOptionPane.showMessageDialog(null, "A criatura "+alvo.getClass().getSimpleName()+ " morreu honrosamente em batalha.");
	}

	public void anunciarUsoDeMagia(Creature caster, Spell magia, Creature alvo, int dano, Status status) {
		JOptionPane.showMessageDialog(null, "O "+caster.getClass().getSimpleName()+" murmurou algumas palavras m�gicas ("+magia.getNome()+"), e a criatura "+alvo.getClass().getSimpleName()+" modificou em "+dano+" seus body points, e conferiu a ele o estado "+status+"!");
	}

	public void anunciarMorteDesafortunada(Creature criatura) {
		JOptionPane.showMessageDialog(null, "Oh não! A criatura "+criatura.getClass().getSimpleName()+" morreu ao pisar numa armadilha!");
	}
}